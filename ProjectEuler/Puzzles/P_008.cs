using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace ProjectEuler.Puzzles
{
    public static class P_008
    {
        public static void Execute()
        {
            string input = @"";

            List<ulong> products = new List<ulong>();

            for (int i = 12; i < input.Length; i++)
            {
                ulong number = ulong.Parse(input[i].ToString())
                            * ulong.Parse(input[i - 1].ToString())
                            * ulong.Parse(input[i - 2].ToString())
                            * ulong.Parse(input[i - 3].ToString())
                            * ulong.Parse(input[i - 4].ToString())
                            * ulong.Parse(input[i - 5].ToString())
                            * ulong.Parse(input[i - 6].ToString())
                            * ulong.Parse(input[i - 7].ToString())
                            * ulong.Parse(input[i - 8].ToString())
                            * ulong.Parse(input[i - 9].ToString())
                            * ulong.Parse(input[i - 10].ToString())
                            * ulong.Parse(input[i - 11].ToString())
                            * ulong.Parse(input[i - 12].ToString());

                products.Add(number);
            }

            Console.WriteLine(products.Max());
        }
    }
}
